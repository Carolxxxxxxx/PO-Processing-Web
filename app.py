...
# ========== **3. 生成 INVOICE & PACKING LIST** ==========
if st.button("🚀 生成 INVOICE 和 PACKING LIST"):
    if not po_uploaded or not price_uploaded:  # **必须上传 PO 和 价格表才允许生成**
        st.error("❌ 请先上传 PO 文件 和 价格表！")
    else:
        os.system(f"python3 template_filler.py {PO_PATH} {PRICE_PATH}")  # **确保读取最新 PO**
