...
def fill_packing_list(template_path, output_path, part_numbers, ordered_quantities, po_number, price_list):
    ...
    for i, (part_number, ordered_quantity) in enumerate(zip(part_numbers, ordered_quantities)):
        ...
        ws[f"A{row}"] = part_number  # 合并单元格填充 Part Number
...
if __name__ == "__main__":
    ...
    price_list_path = "/tmp/Clark11款纸袋报价更新.xlsx"
    invoice_template = "/mount/src/po-processing-web/INVOICE.xlsx"
    packing_list_template = "/mount/src/po-processing-web/PACKING_LIST.xlsx"

    part_numbers, ordered_quantities, po_number = extract_data_from_pdf(po_path)

    if part_numbers and ordered_quantities and po_number:
        price_list = load_price_list(price_list_path)
        fill_invoice(invoice_template, f"/tmp/INVOICE_{po_number}.xlsx", part_numbers, ordered_quantities, po_number, price_list)
        fill_packing_list(packing_list_template, f"/tmp/PACKING_LIST_{po_number}.xlsx", part_numbers, ordered_quantities, po_number, price_list)
